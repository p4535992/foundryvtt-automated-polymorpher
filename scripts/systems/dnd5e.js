...
export default {
	...
	async transformInto(
		...
		if (!arrayMutationNames || arrayMutationNames.length == 0) {
			arrayMutationNames = sourceActor?.getFlag(CONSTANTS.MODULE_NAME, PolymorpherFlags.MUTATION_NAMES_FOR_REVERT) || [];
		}
		...
		if (useWarpGate) {
			 ...
			 */
			//@ts-ignore
			const embeddedFields = Object.values(Actor.implementation.metadata.embedded);
	...
	async revertOriginalForm(sourceToken, sourceActor, renderSheet = true) {
		...
		const previousOriginalActorTokenData = sourceToken.actor?.getFlag(
			CONSTANTS.MODULE_NAME,
        ...
        */
		...
		try {
			if (useWarpGate) {
				...
				// THIS IS THE SOLUTION WITH WARP GATE (AVOID THE CREATION OF ACTOR)
				// ===========================================
				let arrayMutationNames = sourceToken.actor?.getFlag(
					CONSTANTS.MODULE_NAME,
				...
				if (!game.settings.get(CONSTANTS.MODULE_NAME, 'doNotDeleteTmpActors')) {
					...
					const othersActorsToDelete = sourceActor.getFlag(
						CONSTANTS.MODULE_NAME,
						PolymorpherFlags.PREVIOUS_TOKEN_DATA_ORIGINAL_ACTOR
					);
			...
			if (original && hasProperty(original, `flags.${CONSTANTS.MODULE_NAME}`)) {
				...
				for (const token of original.getActiveTokens()) {
					if (token.actor) {
						...
						if (token.actor.getFlag(CONSTANTS.MODULE_NAME, PolymorpherFlags.MUTATION_NAMES_FOR_REVERT) != undefined) {
	...
	async prepareDataFromTransformOptions(
		...
		if (!keepSelf) {
			...
			if (originalActorData.system.abilities) {
