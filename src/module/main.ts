    ...
    ice1: {
      fn: async (template, tokenData) => {
        await new Sequence()
          .effect()
          .file(
            'modules/automated-evocations/assets/animations/JB2A/SnowflakeBurst_01_Regular_BlueWhite_Burst_600x600.webm',
          )
          .belowTokens()
          ...
          .effect()
          .file(
            'modules/automated-evocations/assets/animations/JB2A/IceSpikesRadialBurst_01_Regular_White_1000x1000.webm',
          )
          .randomRotation()
          .atLocation(template)
          .scale(Math.max(tokenData.width, tokenData.height) * tokenData.scale * 0.55)
    ...
    music2: {
      fn: async (template, tokenData) => {
        await new Sequence()
          ...
          .effect()
          .file(
            'modules/automated-evocations/assets/animations/JB2A/BardicInspiration_01_Regular_GreenOrange_400x400.webm',
          )
          .atLocation(template)
          .scale(Math.max(tokenData.width, tokenData.height) * tokenData.scale * 0.45)
          .play();
