...
function buildButton(html, tooltip) {
    ...
    const button = $(`<div class="control-icon ${CONSTANTS.MODULE_NAME}" title="${tooltip}"><i class="${iconClass}"></i></div>`);
...
export function transferItemsActor(sourceActor, targetActor, originalItemId, createdItem, originalQuantity, transferedQuantity, stackItems) {
    ...
    if (transferedQuantity > 0 && transferedQuantity <= originalQuantity) {
        ...
        if (stackItems) {
            const potentialStacks = (targetActor?.items.filter((i) => i.name == originalItem.name && diffObject(createdItem, i) && i.id !== createdItem.id));
...
export async function rollFromString(rollString, actor) {
    ...
    if (!rollString) {
        ...
        if (String(rollString).toLowerCase().includes("data.") ||
...
export async function transferPermissionsActorInner(sourceActor, targetActor, externalUserId) {
    ...
    return await targetActor.update({ ownership: _getHandPermission(sourceActor, externalUserId) }, { diff: false, recursive: false, noHook: true });
}
    ...
    if (original && hasProperty(original, `flags.${CONSTANTS.MODULE_NAME}`)) {
        ...
        for (const token of original.getActiveTokens()) {
            if (token.actor) {
                ...
                if (token.actor.getFlag(CONSTANTS.MODULE_NAME, PolymorpherFlags.PREVIOUS_TOKEN_DATA_ORIGINAL_ACTOR) !=
                    undefined) {
                    const arr = (original.getFlag(CONSTANTS.MODULE_NAME, PolymorpherFlags.PREVIOUS_TOKEN_DATA_ORIGINAL_ACTOR));
                    arr.pop();
