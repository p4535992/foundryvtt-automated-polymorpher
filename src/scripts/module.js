...
export const readyHooks = async () => {
  ...
  ANIMATIONS.animationFunctions = mergeObject(
    ...
    game.settings?.get(CONSTANTS.MODULE_NAME, "customanimations")
  );
  ...
  Hooks.on("renderActorSheet", async function (actorSheet, htmlElement, actorObject) {
    ...
    const settingHudColorButtonRestoreTransformation =
      game.settings.get(CONSTANTS.MODULE_NAME, "hudColorButtonRestoreTransformation") ?? "#d66460";
    if (htmlElement.find(".restore-transformation-pm")?.length > 0) {
      htmlElement.find(
        ".restore-transformation-pm .fa-backward"
      )[0].style.color = `${settingHudColorButtonRestoreTransformation}`;
      htmlElement.find(
        ".restore-transformation-pm .fa-backward"
      )[0].style.textShadow = `0 0 8px ${settingHudColorButtonRestoreTransformation}`;
    }
  });
  ...
  Hooks.on("renderTokenHUD", (app, html, data) => {
    ...
    if (game.settings.get(CONSTANTS.MODULE_NAME, "hudEnable")) {
      ...
      if (html.find(".control-icon.automated-polymorpher .fa-wind")?.length > 0) {
        ...
        html.find(
          ".control-icon.automated-polymorpher .fa-wind"
