...
const API = {
  async invokePolymorpherManagerFromActorArr(...inAttributes) {
    ...
    const result = await this.invokePolymorpherManagerFromActor(
      sourceActorIdOrName,
      removePolymorpher,
      ordered,
      random,
  ...
  async invokePolymorpherManagerArr(...inAttributes) {
    ...
    const result = await this.invokePolymorpherManager(
      sourceTokenIdOrName,
      removePolymorpher,
      ordered,
      random,
  ...
  async revertOriginalForm(sourceToken, sourceActor, renderSheet) {
    let actorOriginalId = await automatedPolymorpherSocket.executeAsGM(
      "revertOriginalForm",
      sourceToken.id,
      sourceActor.id,
      sourceActor.name,
